...
static int replaceFunc(void *addr, void *replace_func, void **old_func) {
    ...
    if (modifyMemAccess(addr, PROT_READ | PROT_WRITE)) {
        DL_DEBUG("[-] modify memAccess fails, error %s.", strerror(errno));
        res = 1;
        goto fails;
    }
...
//        DL_DEBUG("dvmLoadNativeCode Find Hook");
//        MSImageRef image = MSGetImageByName(pathName);
//        if (image != NULL) {
//            void *mFun = MSFindSymbol(image, "unzOpen64");
//            //开始Hook fork
//            if (mFun != NULL) {
//                DL_DEBUG("dvmLoadNativeCode Hook");
//                MSHookFunction(mFun, (void *) &lrand48_hook,
//                               (void **) &old_lrand48);
//            }
//        }
//    }
...
//MSInitialize {
//    __android_log_print(ANDROID_LOG_ERROR, "ZTAG",
//                        "Substrate initialized.");
//    MSImageRef image;
//    image = MSGetImageByName("libnative-lib.so"); //载入lib
//    if (image != NULL) {
//        //注意这个是个c++函数，可以通过objdump来获取
//        void *dvmload = MSFindSymbol(image, "arc4random");
//        if (dvmload == NULL) {
